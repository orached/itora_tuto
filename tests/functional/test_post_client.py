...
def test_managepost_page(setUp, populate_db):
    ...
    response = setUp.get('/managepost', follow_redirects=True)
    assert b"Bonjour john" in response.data
    ...
    assert "Votre article est publié !" in response.data.decode("utf-8")
...
def test_editpost_page(setUp, populate_db):
    ...
    assert "Modifier l'article" in response.data.decode("utf-8")
    ...
    assert response.status_code == 200
    assert "L&#39;article a été mis à jour." in response.data.decode("utf-8")
...
def test_category_page(setUp, populate_db):
    ...
    assert response.status_code == 200
    assert "Catégorie" in response.data.decode("utf-8")
